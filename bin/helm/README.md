...
`Helm` is a lightweight utility designed to discover, inspect, and
...
<table>
  ...
  <tr>
    <td><code>--check</code> / <code>--dry-run</code></td>
    <td>Aliases for one planning operation: summarize pending updates without executing them</td>
  </tr>
  <tr>
...
CI update (`--ci`) distinguishes per-tool records from the summary counts so
scripts can parse both unambiguously:
...
<table>
  ...
  <tr>
    <td><code>--check/--dry-run --json</code></td>
...
`.opencode/bin/helm/CONTRACT.md`). It includes unit tests,
